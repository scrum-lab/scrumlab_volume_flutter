...
enum ShowVolumeUI {
  /// SHOW System UI while changing volume,
  SHOW,
...
/// function.
class Volume {
  static const MethodChannel _channel = MethodChannel('scrumlab_volume');
  ...
  static Future<void> controlVolume(AudioManager audioManager) async {
    final Map<String, int> map = <String, int>{};
    map.putIfAbsent("streamType", () => _getStreamInt(audioManager));
  ...
  static Future<int> get getMaxVol async {
    final int maxVol = await _channel.invokeMethod('getMaxVol');
    return maxVol;
  }
  ...
  static Future<int> get getVol async {
    final int vol = await _channel.invokeMethod('getVol');
    return vol;
  }
  ...
  static Future<int> setVol(int i,
    ...
    final Map<String, int> map = <String, int>{
      'newVol': i,
      'showVolumeUiFlag': _getShowVolumeUiInt(showVolumeUI),
    };
    final int vol = await _channel.invokeMethod('setVol', map);
    return vol;
  }
}
...
int _getStreamInt(AudioManager audioManager) {
  switch (audioManager) {
    ...
    case AudioManager.STREAM_NOTIFICATION:
      return 5;
    default:
      return 3; // Default to STREAM_MUSIC
  }
}
